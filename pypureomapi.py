...
class OmapiMessage(object):
	...
	def dump_oneline(self):
		...
		"""
		return ("authid=%d authlen=%d opcode=%s handle=%d tid=%d rid=%d message=%r obj=%r signature=%r") % (
			self.authid, len(self.signature), repr_opcode(self.opcode), self.handle, self.tid, self.rid, self.message,
...
class InBuffer(object):
	...
	def parse_startup_message(self):
		...
		return parse_map(lambda args: OmapiStartupMessage(*args),
						 parse_chain(self.parse_net32int, lambda _: self.parse_net32int()))
