...
#
# Copyright (C) 2010-2015 Cygnus Networks GmbH
#
...
def repr_opcode(opcode):
    ...
    @rtype: str
    """
    opmap = {1: "open", 2: "refresh", 3: "update",
             4: "notify", 5: "status", 6: "delete"}
...
class OmapiStartupMessage(object):
    ...
    def dump_oneline(self):
        ...
        """
        return "protocol_version=%d header_size=%d" % (
                self.protocol_version, self.header_size)
...
class OmapiHMACMD5Authenticator(OmapiAuthenticatorBase):
    ...
    def sign(self, message):
        ...
        >>> len(OmapiHMACMD5Authenticator(\
                    b"foo", 16*b"x").sign(b"baz")) == authlen
        True
...
class OmapiMessage(object):
    ...
    def verify(self, authenticators):
        ...
        """
        try:
            return authenticators[self.authid]. sign(self.as_string(
                                forsigning=True)) == self.signature
        except KeyError:
            return False
    ...
    def dump_oneline(self):
        ...
        """
        return (("authid=%d authlen=%d opcode=%s handle=%d tid=%d "
                "rid=%d message=%r obj=%r signature=%r")
                % (self.authid, len(self.signature), repr_opcode(self.opcode),
                  self.handle, self.tid, self.rid, self.message, self.obj,
                  self.signature))
...
class InBuffer(object):
    ...
    def parse_net16string(self):
        ...
        return parse_map(operator.itemgetter(1),
                         parse_chain(self.parse_net16int,
                                     self.parse_fixedbuffer))
    ...
    def parse_message(self):
        ...
        parser = parse_chain(self.parse_net32int,  # authid
                            ...
                            lambda *args: self.parse_fixedbuffer(args[1]))
...
class Omapi(object):
    def __init__(self, hostname, port, username=None, key=None):
        ...
        self.protocol.startup_completed = \
            lambda: self.recv_message_queue.append(None)
    ...
    def receive_response(self, message, insecure=False):
        ...
        if not response.is_response(message):
            raise OmapiError("received message is not "
                             "the desired response")
        # signature already verified
        if response.authid != self.protocol.defauth and not insecure:
            raise OmapiError("received message is signed "
    ...
    def add_host_supersede_name(omapi, ip, mac, name):
        """Add a host with a fixed-address and override
        its hostname with the given name.
        @type omapi: Omapi
        @type ip: str
        @type mac: str
        @type name: str
        ...
        msg.obj.append((b"statements",
                        str.encode('supersede host-name "%s";' % name)))
